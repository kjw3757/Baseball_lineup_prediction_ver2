...
# GitHub URL 경로 설정
NC_hitter_path = f"https://raw.githubusercontent.com/kjw3757/Baseball_lineup_prediction_ver2/main/data/NC_2024/NC_hitter_{current_date}.csv"
NC_runner_path = f"https://raw.githubusercontent.com/kjw3757/Baseball_lineup_prediction_ver2/main/data/NC_2024/NC_runner_{current_date}.csv"
NC_defense_path = f"https://raw.githubusercontent.com/kjw3757/Baseball_lineup_prediction_ver2/main/data/NC_2024/NC_defense_{current_date}.csv"
NC_batting_order_path = f"https://raw.githubusercontent.com/kjw3757/Baseball_lineup_prediction_ver2/main/data/NC_2024/NC_batting_order_{current_date}.csv"
NC_players_path = f"https://raw.githubusercontent.com/kjw3757/Baseball_lineup_prediction_ver2/main/data/NC_2024/NC_players_{current_date}.csv"
...
# 데이터 로드
df = load_csv(NC_hitter_path)
columns_to_keep = ['선수명', '팀명', '타율', '경기', '타수', '홈런', '득점', '장타율', '출루율', '득점권타율', '고의4구', '희생플라이', '희생번트']
try:
    df = df[columns_to_keep]
except KeyError as e:
    st.error(f"Column missing in NC_hitter data: {e}")
    st.stop()

df3 = load_csv(NC_runner_path)
try:
    df3 = df3[['선수명', '도루허용', '도루저지']]
except KeyError as e:
    st.error(f"Column missing in NC_runner data: {e}")
    st.stop()
df = pd.merge(df, df3, on='선수명', how='left')

df5 = load_csv(NC_defense_path)
try:
    ...
    st.error(f"Column missing in NC_defense data: {e}")
    st.stop()
...
# 선수명단 로드
players_list = load_csv(NC_players_path)
...
df2 = load_csv(NC_batting_order_path)
